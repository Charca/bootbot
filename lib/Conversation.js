...
class Conversation extends EventEmitter {
  ...
  sendButtonTemplate(text, buttons) {
    return this.bot.sendButtonTemplate(text, buttons, this.userId);
  }
  ...
  sendTemplate(payload) {
    return this.bot.sendTemplate(payload, this.userId);
  }

  sendAttachment(type, url) {
    return this.bot.sendAttachment(type, url, this.userId);
  }

  sendAction(action) {
    return this.bot.sendAction(action, this.userId);
  }

  sendMessage(message) {
    return this.bot.sendMessage(message, this.userId);
  }

  sendRequest(body) {
    return this.bot.sendRequest(body, this.userId);
  }

  sendTypingIndicator(milliseconds) {
    return this.bot.sendTypingIndicator(milliseconds, this.userId);
  }
}
