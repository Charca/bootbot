...
class BootBot extends EventEmitter {
  ...
  set optinOrigin (origin) { this._optinOrigin = origin }
  get optinOrigin () { return this._optinOrigin }
  ...
  _handleEvent(type, event, data) {
    ...
    if (tpye === 'authentication') {
      if (event.sender) {
        ...
        this.optinOrigin = 'checkbox_plugin';
        chat = new Chat(this, event.optin.user_ref);
      }      
    }
    else {
      chat = new Chat(this, event.sender.id);
  ...
  _createRecipient(recipientId) {
    if (this.optinOrigin) {
      let jsonResult = '';
      switch (this.optinOrigin) {
        case 'send_to_messenger': {
          jsonResult = 'recipient: { id: ${recipientId} }'
          break;
        }
        case 'checkbox_plugin': {
          jsonResult = 'recipient: { user_ref: ${recipientId} }'
          break;
        }
        default:
      ...
      return JSON.parse('recipient: { id: ${recipientId} }');
    }
  }
