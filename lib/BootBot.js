...
class BootBot extends EventEmitter {
  ...
  setGreetingText(defaultText, localizedTexts) {
    return this.sendProfileRequest({
      greeting: [{
          locale: 'default',
          text: defaultText
        }].concat(localizedTexts)
    });
  }
  ...
  setPersistentMenu(defaultButtons, defaultComposerInputDisabled, localizedMenues) {
    const formattedButtons = this._formatButtons(defaultButtons);
    return this.sendProfileRequest({
      persistent_menu: [{
        locale: 'default',
        composer_input_disabled: defaultComposerInputDisabled,
        call_to_actions: formattedButtons
      }].concat(localizedMenues)
    });
  }
