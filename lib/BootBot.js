...
class BootBot extends EventEmitter {
  ...
  getWorkplaceUserProfile(userId) {
    // FB Workplace has a slightly different user format
    const url = `https://graph.facebook.com/v2.6/${userId}?fields=first_name,last_name,picture,locale,timezone,gender&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
      .catch(err => console.log(`Error getting workplace user profile: ${err}`));
  }
