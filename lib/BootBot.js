...
const user_views = require('../../../views/bot');
...
class BootBot extends EventEmitter {
  ...
  getFBInfo(userId, callback) {
    request({
        url: `https://graph.facebook.com/v2.6/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${this.accessToken}`,
        json:true,
        method : "GET"
    }, callback);
  }
  ...
  _handleEvent(type, event, data) {
    ...
    // user_models.get(recipient, event, this);
  ...
  handleFacebookData(data) {
    ...
    data.entry.forEach((entry) => {
      ...
      entry.messaging.forEach((event) => {
        ...
        // handleEvent:
        // let user_id = (type === 'authentication' && !event.sender) ? { user_ref: event.optin.user_ref } : event.sender.id;
        ...
        // callback: provide a function argument to another function
        // pass the function parseICM as a param to get method
        user_models.get(user_id, event, user_views.parseICM);
      });
    });
  }
