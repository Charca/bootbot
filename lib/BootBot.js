...
class BootBot extends EventEmitter {
   ...
   * @param {String} [options.database=./{db}.json]
   * @param {String} [options.databaseDefault=system]
   */
  constructor(options) {
    super();
    ...
    this.database = options.database || './{db}.json';
    this.databaseDefault = options.databaseDefault || 'system';
    this.app = express();
    ...
    this._hearMap = [];
    this._conversations = [];
    this.db = this._db(this.databaseDefault);
  }
  ...
  _db(db) {
    let adapter = new FileSync(this.database.replace('{db}', db));
    return low(adapter);
  }
}
